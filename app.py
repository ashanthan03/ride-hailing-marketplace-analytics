...
allocator_choice = st.sidebar.radio(
    "Allocator", ["Rule-Based", "Adaptive (Bandit/RL)"]
)
use_rl = allocator_choice == "Adaptive (Bandit/RL)"

if st.button("Run Simulation"):

    df, surge_df, kpis, riders_df, allocator_type = run_simulation(
        num_zones,
        num_riders,
        num_requests,
        time_steps,
    ...
    run_id = save_run(conn, config, kpis, df, riders_df, surge_df)
    conn.close()
    st.caption(f"Saved as run #{run_id} ({allocator_type})")
    ...
    for r in rider_range:
        _, _, kpis_sweep, _, _ = run_simulation(
            num_zones,
            r,
            num_requests,
            time_steps,
        ...
        revenues.append(kpis_sweep["revenue"])
